...
class ModelSearch2(object):
    ...
    def get_solution_details(self, soln_ids):
        solns = {}
    ...
    def fit_solutions(self, solns, ds, prob):
        # Get training dataset
        train_ds = ds.get_training_dataset()
        # Get fitted solution
        fit_req_ids = {}
        ...
        return fitted_models, mdl_predicitions

    def get_model_predictions(self, fitted_models, ds, prob):
    ...
    def rank_scored_models(self, solns, model_scores, ds, prob):
        ...
        for row in sorted_data.iterrows():
            ...
            ranked_models[rm._id] = rm

        return ranked_models


    def short_run(self, ds, prob, out_path=None):
        logger.info("Running Shortened Pipeline Search on TA2")
        ...
        # Search for solutions
        soln_ids = self.search(ds, prob)

        # Get Model for each solution returned
        solns = self.get_solution_details(soln_ids)
        ...
        # Get Score for each solution
        model_scores = self.score_models(solns, ds, prob)
        ...
        logger.debug("********************************")
        logger.debug("Done scoring solutions")
        logger.debug("********************************")

        ranked_models = self.rank_scored_models(solns, model_scores, ds, prob)
        rm_ids = [rmid for rmid, rm in ranked_models.items()]
    ...
    def run(self, ds, prob, out_path=None):
        ...
        # Get fitted solution
        fitted_models, mdl_predictions = self.fit_models(solns, ds, prob)
        ...
        logger.debug("********************************")
        logger.debug("Done fitting models")
        logger.debug("********************************")

        # Get Model Predictions on test data
        test_predictions = self.get_model_predictions(fitted_models, ds, prob)

        # Update Workflow Session with model predictions
        ...
        logger.debug("********************************")
        logger.debug("Done getting model predictions on test data")
        logger.debug("********************************")
