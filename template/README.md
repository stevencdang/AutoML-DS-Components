...
```bash
./runWCC.sh . <path_to_component.git>/wcc.properties
cd <path_to_component.git>
cp <path_to_component.git>/install_component.sh .
cp <path_to_component.git>/README.md .
cp <path_to_component.git>/requirements.txt .
...
In order to reinstall due to errors in the install script you simply have to `rm -R env` to delete the virtualenv.
